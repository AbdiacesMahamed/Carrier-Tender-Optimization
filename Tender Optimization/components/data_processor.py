...
def apply_volume_weighted_performance(merged_data):
    ...
    if 'Performance_Score' not in merged_data.columns:
        st.warning("⚠️ No Performance_Score column found - skipping performance calculation")
        return merged_data
    ...
    if missing_before == 0:
        ...
        st.info("✅ All performance scores already complete - no processing needed")
        return merged_data
    ...
    # Display summary
    if missing_after > 0:
        st.error(f"❌ Still have {missing_after} missing performance scores after processing!")
    else:
        st.success(f"✅ Performance scores completed: {filled_count} values filled across {len(carrier_weighted_performance)} carriers")
...
def process_performance_data(Performancedata, has_performance):
    ...
    try:
        ...
        if len(performance_filled) > 0:
            ...
            st.success(f"✅ Performance data processed: {len(performance_filled)} records from {len(week_columns)} weeks")
            if filled_count > 0:
                st.info(f"📊 Filled {filled_count} missing performance scores using volume-weighted averages")
